package com.lingzhen.myproject.englishword.plugin;

import com.lingzhen.myproject.englishword.util.PageData;
import com.lingzhen.myproject.englishword.util.ReflectUtil;
        ...
        if (iv.getTarget() instanceof RoutingStatementHandler) {
            ...
            if (mappedStatement.getId().matches(sqlId)) {
                ...
                if (null == parameterObject) {
                    ...
                    countStmt.close();
                    PageData pd = null;
                    if (parameterObject instanceof PageData) {
                        pd = (PageData) parameterObject;
                        pd.put("sql_count",count);
                    } else {
                        throw new Exception(mappedStatement.getId()+"的参数必须为PageData");
                    }
                    //组装分页SQL
